...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockDeepLinkService.generateJoinLink(any())).thenAnswer(
      (inv) => 'https://ironsplit.app/join?code=${inv.positionalArguments.first}',
    );
  });
  ...
  group('S54TaskSettingsInvitePage widget test', () {
    testWidgets('no active invite should auto-generate new invite code', (tester) async {
      when(() => mockTaskRepo.streamTask('task-1')).thenAnswer(
        (_) => Stream.value(_task(activeInviteCode: null, activeInviteExpiresAt: null)),
      );
    ...
    testWidgets('tap Regenerate should request new code and update display', (tester) async {
      when(() => mockTaskRepo.streamTask('task-1')).thenAnswer(
        (_) => Stream.value(
          _task(
            activeInviteCode: 'WXYZ5678',
            activeInviteExpiresAt: DateTime.now().add(const Duration(minutes: 10)),
          ),
        ),
      );
      when(() => mockInviteRepo.createInviteCode('task-1')).thenAnswer(
    ...
    testWidgets('tap Share should call shareText with invite code and link', (tester) async {
      when(() => mockTaskRepo.streamTask('task-1')).thenAnswer(
        (_) => Stream.value(
          _task(
    ...
    testWidgets('tap invite code should copy raw code to clipboard', (tester) async {
      when(() => mockTaskRepo.streamTask('task-1')).thenAnswer(
        (_) => Stream.value(
          _task(
    ...
    testWidgets('expired invite should disable share and show expired label', (tester) async {
      when(() => mockTaskRepo.streamTask('task-1')).thenAnswer(
        (_) => Stream.value(
          _task(
            activeInviteCode: 'TIME1234',
            activeInviteExpiresAt: DateTime.now().add(const Duration(seconds: 1)),
          ),
        ),
      );
